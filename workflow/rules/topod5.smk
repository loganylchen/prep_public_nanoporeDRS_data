rule pod5_f2p:
    ...
    output:
        raw_pod5_dir=temp(directory("data/tmp.{sample}.pod5")),
        tag=temp("data/tmp.{sample}.pod5.tag"),
    ...
    shell:
        "pod5 convert fast5 -t {threads} -f -r -o {output.raw_pod5_dir} {input.raw_fast5_dir}  1>{log.log} 2>{log.err} && "
        "touch {output.tag}"
